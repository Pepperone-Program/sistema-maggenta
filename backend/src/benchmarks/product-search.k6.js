...
const queries = [
  ...
  'personalizado',
  'bloco com pauta',
  'garrafa parede dupla',
  'garrafa térmica inox 500ml',
  'mochila notebook 15.6"',
  'A5',
  'UV',
];
...
export default function () {
  ...
  const response = http.get(
    `${baseUrl}/api/v1/produtos/site/busca?empresaId=${tenantId}&q=${encodeURIComponent(query)}&limit=24${cacheBuster}`,
    {
  ...
  check(response, {
    ...
    'contract items': (result) => Boolean(result.json('data.items')),
  });
}

export function handleSummary(data) {
  return {
    'search-benchmark-report.json': JSON.stringify(
      {
        generatedAt: new Date().toISOString(),
        environment: {
          baseUrl,
          tenantId,
          cacheMode: __ENV.SEARCH_CACHE_MODE || 'warm',
          hardware: __ENV.SEARCH_HARDWARE || 'NOT_RECORDED',
          replicas: __ENV.SEARCH_REPLICAS || 'NOT_RECORDED',
          dataset: __ENV.SEARCH_DATASET || 'NOT_RECORDED',
        },
