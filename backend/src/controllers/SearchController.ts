...
const fail = (res: Response, error: unknown): void => {
  ...
  errorResponse(
    ...
    err.message || 'Erro no mecanismo de busca',
    err.statusCode || 500,
  );
};
const idParam = (value: string, name = 'id'): number => {
  ...
  if (!Number.isInteger(parsed) || parsed <= 0)
    throw Object.assign(new Error(`${name} invalido`), { code: 'INVALID_SEARCH_ID', statusCode: 422 });
  return parsed;
};
...
export class SearchController {
  static async autocomplete(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const items = await SearchAutocompleteService.autocomplete(
        empresaId,
        String(req.query.q || ''),
  ...
  static async click(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      if (
        !/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(searchId) ||
        ...
        errorResponse(
          res,
          'INVALID_SEARCH_CLICK',
          'searchId, productId e position validos sao obrigatorios',
          422,
        );
        return;
      }
  ...
  static async list(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      if (!entities.has(entity))
        throw Object.assign(new Error('Entidade de busca invalida'), {
          code: 'INVALID_SEARCH_ENTITY',
          statusCode: 404,
        });
  ...
  static async create(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      successResponse(
        res,
        await SearchAdminService.create(req.user!.id_empresa, entity, req.body),
        'Registro criado',
        201,
      );
    } catch (error) {
  ...
  static async update(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      successResponse(
        res,
        await SearchAdminService.update(req.user!.id_empresa, entity, idParam(req.params.id), req.body),
        'Registro atualizado',
      );
    } catch (error) {
      fail(res, error);
  ...
  static async remove(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      await SearchAdminService.remove(
        req.user!.id_empresa,
        entity,
        idParam(req.params.id),
        req.query.conflictingOptionId
          ? idParam(String(req.query.conflictingOptionId), 'conflictingOptionId')
          : undefined,
      );
  ...
  static async getMetadata(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      successResponse(
        res,
        await SearchAdminService.getProductMetadata(req.user!.id_empresa, idParam(req.params.id)),
      );
    } catch (error) {
      fail(res, error);
    }
  ...
  static async putMetadata(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      successResponse(
        res,
        await SearchAdminService.replaceProductMetadata(
          req.user!.id_empresa,
          idParam(req.params.id),
          req.user!.id_usuario,
          req.body.attributes || [],
  ...
  static async debug(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const sort: SearchSort = ['newest', 'popular'].includes(String(req.query.sort))
        ? (String(req.query.sort) as SearchSort)
        : 'relevance';
      const ranked = ProductRankingEngine.rank(retrieval.candidates, intent, sort)
        .slice(0, 50)
        .map((item) => ({
          ...
          coverage: item.lexicalCoverageRatio,
          ordering: SearchCursorCodec.tuple(item),
        }));
      successResponse(res, {
        normalized,
        intent,
