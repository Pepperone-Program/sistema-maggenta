...
const readyCatalogs = new Map<number, number>();
...
export class SearchDictionaryService {
  ...
  static async assertCatalogReady(empresaId: number): Promise<void> {
    if ((readyCatalogs.get(empresaId) || 0) > Date.now()) return;
    ...
    readyCatalogs.set(empresaId, Date.now() + SEARCH_CACHE.dictionaryTtlMs);
  }
  ...
  static async getEntries(
    ...
    const usableCachedVersion =
      cachedVersion && cachedVersion.expiresAt > Date.now() ? cachedVersion.value : undefined;
    const version = knownVersion || usableCachedVersion;
    const cached = cache.get(empresaId);
    const expectedCacheVersion = version
      ? `${version.catalogVersion}:${version.dictionaryVersion}`
      : undefined;
    ...
    const manualProductTerms = new Set(
      manualEntries.filter((entry) => entry.termType === 'PRODUCT_TYPE').map((entry) => entry.normalizedTerm),
    );
    ...
    const entries = [...manualEntries, ...automaticEntries].sort(
      (left, right) =>
        right.tokenCount - left.tokenCount || right.priority - left.priority || left.id - right.id,
    );
    cache.set(empresaId, {
      expiresAt: Date.now() + SEARCH_CACHE.dictionaryTtlMs,
